...
const Leads: React.FC = () => {
  ...
  useEffect(() => {
    const tagData = async () => {
      try {
        ...
        const statusesResponse = await callApi(
          METHOD.GET,
          `crm/status/?masterId=${leadId}`
        );
        const tagCategories = tagCategoryResponse?.data;
        const statusCategories = statusesResponse?.data;
        setTagsCategories(tagCategories);
        setStatuses(statusCategories);
      } catch (error) {
  ...
  //           formMode: "edit",
  //           formValues:
  //             data?.find((lead: { id: string }) => lead.id === leadId) ||
  //             initialValues,
  //         }));
  //       } catch (error) {
            ...
            {filteredLeads && filteredLeads.length > 0 ? (
              <table className="table-auto w-full mt-5 text-sm border-collapse">
                ...
                <tbody>
                  {filteredLeads
                    ?.slice(
                      (state.currentPage - 1) * state.leadsPerPage,
                      state.currentPage * state.leadsPerPage
                    )
                    .map((lead) => (
                      <LeadsData
                        statuses={statuses}
